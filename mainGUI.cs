...
#region extended_aux_states
        ...
        private void update_gui()
        {
            ...
            lVBatConf.Text = String.Format("{0:0.0} 伏", (double)mw_gui.vBat / 10);
            ...
            if (gui_settings.speech_enabled)
            {
                if ((prev_state != mw_gui.nav_state) || (prev_wp != mw_gui.wp_number))
                {
                    ...
                    {
                        case 1: speech.SpeakAsync("开始自动返航.");
                            break;
                        case 2: speech.SpeakAsync("正在前往返回地点.");
                            break;
                        case 3: speech.SpeakAsync("高度保持模式开启.");
                            break;
                        case 4: speech.SpeakAsync("正在悬停等待.");
                            break;
                        case 5: speech.SpeakAsync("正在前往第" + mw_gui.wp_number.ToString() + "个航点.");
                            break;
                        case 9: speech.SpeakAsync("自动着陆开始.");
                            break;
                        case 10: speech.SpeakAsync("飞行器已经降落.请将油门收到最小，并锁定飞行器.");
                            break;
            ...
            if (tabMain.SelectedIndex == GUIPages.FlightDeck)
            {
                ...
                l_cycletime.Text = String.Format("{0:0000} 微秒", mw_gui.cycleTime);
                l_vbatt.Text = String.Format("{0:0.0} 伏", (double)mw_gui.vBat / 10);
        ...
        private void b_cal_acc_Click(object sender, EventArgs e)
        {
            ...
            if (!isConnected)
            {
                MessageBoxEx.Show(this, "必须先连接飞控!", "Not connected", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (MessageBoxEx.Show(this, "请确保飞行器绝对的水平!\r\n然后点击OK按钮开始校准,请保持水平状态5秒.", "校准加速度计", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
            {
        ...
        private void b_cal_mag_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBoxEx.Show(this, "在按下OK按钮后，你有一分钟时间来完成以下操作：\r\n旋转你的磁力计360度（3轴均要旋转）", "校准磁力计", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
            {
        ...
        private void b_read_settings_Click(object sender, EventArgs e)
        {
            if (isConnected)
            {
                ...
                if (missing_packets) MessageBoxEx.Show("响应包不完整,\r，为了软件参数保持与飞控一致，请重新读取数据！", "响应包丢失", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        private void b_save_gui_settings_Click(object sender, EventArgs e)
        {
            ...
            if (bRestartNeeded)
            {
                MessageBoxEx.Show(this, "改变MultiWii飞控的软件版本，您必须重新启动此软件", "MWWGUI Restart needed", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                Environment.Exit(-1);
            }
        }
            ...
            if (!isMouseDown)
            {
                ...
                if (GMRouteMission != null)
                {
                    if (GMRouteMission.Points.Count >= 1)
                    {
                        ...
                        lDistLastWP.Text = String.Format("距离选中点:{0:N1}米", dist_from_last * 1000);
                    }
                }
            }
                ...
                else
                {
                    ...
                    if (pIndex.HasValue)
                    {
                        if (pIndex < GMRouteMission.Points.Count)
                        {
                            ...
                            MainMap.UpdateRouteLocalPosition(GMRouteMission);
                            lDistance.Text = String.Format("任务总距离:{0:N1}米", GMRouteMission.Distance * 1000);
                            ...
                            if (pIndex == 0)            //Dragging the first marker
                            {
                                if (GMRouteMission.Points.Count > 1)  //We have more than 1 WP
                                {
                                    ...
                                    lDistLastWP.Text = String.Format("下一任务点距离:{0:N1}米", dist_from_last * 1000);
                                }
                            }
                            else                        //Drag some other marker
                            {
                                ...
                                lDistLastWP.Text = String.Format("上一任务点距离:{0:N1}米", dist_from_last * 1000);
                            }
        ...
        private void fetch_map()
        {
            ...
            if (area.IsEmpty)
            {
                DialogResult res = MessageBox.Show("没有明确区域，保存屏幕区域？", "Rip", MessageBoxButtons.YesNo);
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            if (isConnected)
            {
                MessageBoxEx.Show(this, "请在进入日志下载模式前断开飞行控制器", "请先断开连接", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void addWP(string action, int P1, int P2, int P3, double Lat, double Lon, int Alt)
        {
            ...
            {
                MessageBox.Show("不能增加任务。最大任务数(" + Convert.ToString(mw_gui.max_wp_number) + ") 到达", "Max steps reached", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void updateMap()
        {
            ...
            lDistance.Text = String.Format("任务总距离:{0:N1} m", GMRouteMission.Distance * 1000);
        }
        ...
        private void change_datagrid_header(string sAction)
        {
            if (sAction == "WAYPOINT" || sAction == "POSHOLD_UNLIM")
            {
                ...
                missionDataGrid.Columns[5].HeaderText = "纬度";
                missionDataGrid.Columns[6].HeaderText = "经度";
                missionDataGrid.Columns[7].HeaderText = "高度";
            }
            if (sAction == "POSHOLD_TIME")
            {
                missionDataGrid.Columns[2].HeaderText = "角度";
                missionDataGrid.Columns[3].HeaderText = "";
                missionDataGrid.Columns[4].HeaderText = "";
                missionDataGrid.Columns[5].HeaderText = "纬度";
                missionDataGrid.Columns[6].HeaderText = "经度";
            ...
            if (sAction == "RTH")
            {
                missionDataGrid.Columns[2].HeaderText = "降落";
                missionDataGrid.Columns[3].HeaderText = "";
                missionDataGrid.Columns[4].HeaderText = "";
            ...
            if (sAction == "JUMP")
            {
                missionDataGrid.Columns[2].HeaderText = "编号#";
                missionDataGrid.Columns[3].HeaderText = "重复数";
                missionDataGrid.Columns[4].HeaderText = "";
                missionDataGrid.Columns[5].HeaderText = "";
                missionDataGrid.Columns[6].HeaderText = "";
            ...
            if (sAction == "SET_HEAD")
            {
                missionDataGrid.Columns[2].HeaderText = "航向";
                missionDataGrid.Columns[3].HeaderText = "";
                missionDataGrid.Columns[4].HeaderText = "";
                missionDataGrid.Columns[5].HeaderText = "";
                missionDataGrid.Columns[6].HeaderText = "";
        ...
        private void txtDefAlt_TextChanged(object sender, EventArgs e)
        {
            ...
            if (i < 2 || i > 400)
            {
                MessageBoxEx.Show(this, "默认高度必须在 2米到400米之间", "Altitude error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        private bool loadMission(string filename)
        {
            ...
            {
                MessageBoxEx.Show("选择的任务文件出现的错误行: " + reader.LineNumber, "Invalid Data", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return (false);
            }
        ...
        private void bBind_Click(object sender, EventArgs e)
        {
            if (!isConnected)
            {
                MessageBoxEx.Show(this, "请先连接飞控！", "未连接", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (MessageBoxEx.Show(this, "按下确定按钮后，你的遥控器接收机将进入绑定模式。\r\nAfter pressing OK your Spektrum receiver will be in bind mode, power cycle when bind is finished", "Bind Spektrum receiver", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
            {
        ...
        private void btnUploadMission_Click(object sender, EventArgs e)
        {
            ...
            if (missionDataGrid.Rows.Count == 0)
            {
                ...
                {
                    MessageBox.Show("飞控无应答，任务上传中止。再次尝试读取任务.", "应答超时", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }

                if (qStatus == WP_Query.Error)
                {
                    MessageBox.Show("上传发生错误, 可能是在飞行中、降落中, 请锁定飞行器再重试.", "上传错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            ...
            for (a = 0; a < missionDataGrid.Rows.Count - 0; a++)
            {
                ...
                if (qStatus == WP_Query.Timeout)
                {
                    MessageBox.Show("飞控无应答, 任务上传终止. 请重新读取任务.", "应答超时", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
                }

                if (qStatus == WP_Query.Error)
                {
                    MessageBox.Show("上传错误, 可能正在飞行中、降落中, 请锁定飞行器再试.", "上传错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
                }
            ...
            {
                MessageBox.Show("任务成功上传。\r\n建议重新下载任务,并进行校对.", "上传完成", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void btnDownLoadMission_Click(object sender, EventArgs e)
        {
            ...
            while (!finished)
            {
                ...
                {
                    finished = true;
                    MessageBox.Show("飞控无应答，任务列表可能不完整，请重新读取任务列表.", "应答超时", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }

                if (qStatus == WP_Query.Error)
                {
                    finished = true;
                    MessageBox.Show("发生错误, 可能正在飞行中、降落中, 请锁定飞行器再试.", "下载任务失败", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }

                if (qStatus == WP_Query.CRC)
                {
                    finished = true;
                    MessageBox.Show("EEPROM读取数据CRC校验错误，任务描述被损坏，请重新上传.", "CRC校验错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                ...
                if (i == 255)
                {
                    ...
                    missionDataGrid.Rows.Clear();
                    updateMap();
                    MessageBox.Show("下载超限，收到无效的响应，请检查通信", "下载错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
        ...
        private void tsMenuAddPosholdTimed_Click(object sender, EventArgs e)
        {
            ...
            string time = "20";
            int iTime = 20;
            if (System.Windows.Forms.DialogResult.Cancel == InputBox.Show("位置保持点", "保持的秒数", ref time))
                return;
        ...
        private void tsMenuAddJump_Click(object sender, EventArgs e)
        {
            ...
            string wp = "1";
            if (System.Windows.Forms.DialogResult.Cancel == InputBox.Show("路径编号", "跳转到指定的路径编号?", ref wp))
                return;
            string repeat = "5";
            if (System.Windows.Forms.DialogResult.Cancel == InputBox.Show("跳转重复", "重复次数（-1指永远）", ref repeat))
                return;
